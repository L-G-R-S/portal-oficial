...
export default function ProspectDetail() {
  ...
  const { isLoading, data, reload } = useEntityDetail("prospect", id);
  ...
  if (isLoading) {
    return (
      <div className="space-y-6">
        <Button variant="outline" onClick={() => navigate(ROUTES.PROSPECTS)}>
  ...
  if (!data) {
    return (
      <div className="space-y-6">
        ...
        <Card>
          <CardContent className="p-12 text-center">
            <p className="text-muted-foreground">Prospect não encontrado.</p>
          </CardContent>
        </Card>
      </div>
    );
