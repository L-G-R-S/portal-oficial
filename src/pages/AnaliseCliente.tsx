...
export default function AnaliseCliente() {
  ...
  const currentAnalysis = Array.from(analyses.values()).find(a => a.entityType === 'client' && a.status === 'analyzing');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    startAnalysis(cleanedDomain, 'client');
  };

  return (
    <div className="space-y-6">
      <AnalysisProgressBanner entityType="client" />
      ...
      <div>
        <h1 className="text-3xl font-bold text-foreground mb-2">Análise de Cliente</h1>
        <p className="text-muted-foreground">
          Insira o domínio da empresa para realizar uma análise completa do cliente
        </p>
      </div>
        ...
        <CardContent>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-muted-foreground">
                Receba insights completos sobre o cliente
              </p>
            </div>
          </div>
        </CardContent>
