  ...
  return (
    <Card className="p-6 bg-gradient-to-br from-secondary to-secondary/80 text-secondary-foreground overflow-hidden relative">
      ...
      <div className="relative z-10">
          ...
          <div className="flex items-center gap-4 flex-1">
            {company.logo_url ? (
              <img 
                ...
                className="w-16 h-16 rounded-lg object-contain bg-background/10 p-1"
              />
            ) : (
              <div className="w-16 h-16 rounded-lg bg-background/10 flex items-center justify-center">
                <Building2 className="h-8 w-8" />
              </div>
            )}
            <div>
              <h2 className="text-xl font-bold">{company.name || 'Sua Empresa'}</h2>
              {company.industry && (
                <Badge variant="outline" className="mt-1 border-secondary-foreground/30 text-secondary-foreground/80">
                  {company.industry}
                </Badge>
              )}
            </div>
        ...
        {/* Ranking Badge */}
        {linkedinRank && totalCompetitors && (
          <div className="mt-4 pt-4 border-t border-secondary-foreground/20">
            <p className="text-sm text-secondary-foreground/80">
              Você está em <span className="font-bold text-primary">{linkedinRank}º lugar</span> no LinkedIn entre {totalCompetitors + 1} empresas monitoradas
            </p>
          </div>
        )}
      </div>
